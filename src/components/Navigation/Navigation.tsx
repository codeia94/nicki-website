  ...
  const scrollToSection = (sectionId: string) => {
	...
	return (
		...
		<header className={`fixed z-50 top-0 text-black flex justify-center w-screen font-oldStandardReg  `}>
			<nav className={`${isMenuOpen ? 'flex flex-col-reverse bg-sky-100' : 'px-6 pt-4 flex justify-between w-screen max-w-5xl items-center' }`}>
			{!isMenuOpen && (
				<Link href='/'>
					<MySvgIcon href="/" className='w-14 h-14' />
				</Link>
			)}
